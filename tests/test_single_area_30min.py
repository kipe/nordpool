...
class ElspotTestCase(unittest.TestCase):
    ...
    def test_single_area_30min(self):
        with vcr.use_cassette("single_area_30min.yaml"):
            elspot = Prices()
            prices = elspot.fetch(end_date="2025-05-07", areas=["FI"], resolution=30)
            self.assertEqual(
                prices,
                {
                    "areas": {
                    ...
                    "updated": datetime.datetime(
                        2025, 5, 6, 11, 22, 10, 872203, tzinfo=utc
                    ),
                },
            )
